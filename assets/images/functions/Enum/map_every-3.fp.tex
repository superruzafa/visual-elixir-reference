\matrix [list=a] {
  ...
  \node (a1nth-1) {$a_{\scriptsize\texttt{nth}-1}$}; &
  \node (a1nth) {$a_{\scriptsize\texttt{nth}}$}; &
  \node [elements between=1]; &
  \node (ainth-1) {$a_{i\cdot\scriptsize\texttt{nth}-1}$}; &
  \node (ainth) {$a_{i\cdot\scriptsize\texttt{nth}}$}; &
  \node [elements between]; &
  \node [index=n]; \\
};
...
\matrix [list=b, below=2 of list a] {
  ...
  \node (b1) {$a_1$}; &
  \node [elements between]; &
  \node (b1nth-1) {$a_{\scriptsize\texttt{nth}-1}$}; &
  \node (b1nth) {$b_{\scriptsize\texttt{nth}}$}; &
  \node [elements between=1]; &
  \node (binth-1) {$a_{i\cdot\scriptsize\texttt{nth}-1}$}; &
  \node (binth) {$b_{i\cdot\scriptsize\texttt{nth}}$}; &
  \node [elements between]; &
  \node [index=n]; \\
};
