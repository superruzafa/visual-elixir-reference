...
\matrix[tuple=n, right=1 of tuple 2, element size=\smalllistitemsize] {
    \node (tn1) [placeholder=2.5]; & \node [separator]; &
    \node (accn) {$acc_n$}; \\
};

\matrix at (tn1) [list=b3, element=b] {
    \node (bn1) {$b_{n_1}$}; &
    \node [elements between]; &
    \node (bnm) {$b_{n_m}$}; \\
};

\matrix [tuple=n+1, right=of tuple n, element size=\smalllistitemsize] {
    \node (tn+11) [placeholder=2.5]; & \node [separator]; &
    \node (accn+1) {$acc_{n+1}$}; \\
};

\matrix at (tn+11) [list=b4, element=b] {
    \node (bn+11) {$b_{{n+1}_1}$}; &
    \node [elements between]; &
    \node (bn+1k) {$b_{{n+1}_k}$}; \\
};
